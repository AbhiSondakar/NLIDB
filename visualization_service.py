...
class VisualizationService:
    ...
    def analyze_data_for_viz(self, df: pd.DataFrame) -> dict:
        ...
        if len(df) > 10000:
            return {
                'can_visualize': False,
                'reason': f'Dataset too large ({len(df)} rows). Maximum 10,000 rows allowed.',
                'recommended_chart': None
            }
        ...
        if len(numeric_cols) >= 3:
            if len(df) >= 10:  # Need enough data for correlation
                return {
                    'can_visualize': True,
    ...
    def create_bar_chart(self, df: pd.DataFrame, x: str, y: str, title: str) -> str:
        ...
        if df[x].nunique() > 20:
            df = df.nlargest(20, y)
            title = f"{title} (Top 20 of {original_count} categories)"
    ...
    def create_pie_chart(self, df: pd.DataFrame, labels: str, values: str, title: str) -> str:
        ...
        if len(df) > 10:
            df = df.nlargest(10, values)
            title = f"{title} (Top 10 of {original_count} items)"
